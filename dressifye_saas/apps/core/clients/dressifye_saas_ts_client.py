"""
packages/dressifye_saas TypeScript API istemcisi.
...
class DressifyeSaasTSClient:
    """
    packages/dressifye_saas TypeScript API ile iletişim kurar.

    Env / settings gereksinimleri:
      GARMENT_CORE_TS_URL         → TS servisinin base URL'i (ör. https://dressifye-saas-ts.railway.app)
    ...
    def push_garment(
        ...
        """
        if not self.enabled:
            logger.debug("DressifyeSaasTSClient devre dışı (GARMENT_CORE_TS_ENABLED=False)")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error("DressifyeSaasTS HMAC doğrulama başarısız")
                return {"success": False, "error": "Geçersiz imza"}
            elif response.status_code == 422:
                data = response.json()
                logger.warning("DressifyeSaasTS güvenlik reddi: %s", data.get("verdict"))
                return {
                    "success": False,
            ...
            logger.error("DressifyeSaasTS zaman aşımı")
            return {"success": False, "error": "Zaman aşımı"}
        except requests.ConnectionError:
            logger.error("DressifyeSaasTS bağlantı hatası: %s", self.base_url)
            return {"success": False, "error": "Bağlantı kurulamadı"}
        except Exception as e:
            logger.exception("DressifyeSaasTS beklenmeyen hata: %s", e)
