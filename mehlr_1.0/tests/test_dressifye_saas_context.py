"""
dressifye-saas → Mehlr AI bağlam kaynağı testleri.
...
from mehlr.integrations.dressifye_saas_context_source import (
...
class FetchGarmentsFromCoreTests(TestCase):
    @patch("mehlr.integrations.dressifye_saas_context_source.urllib.request.urlopen")
...
@override_settings(**GC_SETTINGS)
class GetDressifyeContextWithDressifyeSaasTests(TestCase):
    @patch("mehlr.integrations.dressifye_saas_context_source.fetch_garments_from_core")
    @patch("mehlr.services.context_manager.DressifyeClient")
    def test_falls_back_to_dressifye_saas_when_dressifye_empty(
        ...
        mock_fetch.return_value = [
            {
                ...
                "size": "",
                "image_url": "",
                "metadata": {"fabricType": "Pamuk", "source": "dressifye_saas"},
            }
        ]
    ...
    @patch("mehlr.integrations.dressifye_saas_context_source.fetch_garments_from_core")
    @patch("mehlr.services.context_manager.DressifyeClient")
    def test_dressifye_takes_priority_over_dressifye_saas(
        self, mock_client_cls, mock_fetch
    ) -> None:
        from mehlr.services.context_manager import get_dressifye_context
