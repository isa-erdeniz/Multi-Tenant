"""
Celery görevleri — Mehlr → dressifye-saas köprüsü.
...
from mehlr.integrations.dressifye_saas_client import send_to_dressifye_saas

logger = logging.getLogger("mehlr.tasks_dressifye_saas")
...
def push_garment_to_core(
    ...
    """
    Tek bir DressifyeGarment kaydını dressifye-saas'a ingest eder.
    Celery retry mekanizması ile geçici hatalara karşı dayanıklıdır.
    """
    try:
        result = send_to_dressifye_saas(
...
def push_outfit_recommendation_to_core(
    ...
    """
    OutfitRecommendation dressifye-saas'a bildirilir; `synced_to_dressifye` işaretlenir.
    """
