...
logger = logging.getLogger("mehlr.integrations.dressifye_saas_context")
...
def fetch_garments_from_core(
    ...
    """
    dressifye-saas GET /v1/garments?tenant_slug=...&limit=...
    ...
    try:
        ...
        logger.warning("dressifye_saas_context_source HTTP %s tenant=%s", exc.code, tenant_slug)
    except Exception as exc:
        logger.warning("dressifye_saas_context_source hata tenant=%s: %s", tenant_slug, exc)
    return []


def _map_to_dressifye_shape(g: dict[str, Any]) -> dict[str, Any]:
    """
    dressifye-saas satırını DressifyeClient / context_manager normalizer'ı ile uyumlu şekle çevirir.
    ...
    for field in ("fabricType", "hairType", "styleEra", "cosmeticFinish", "colorPalette"):
        ...
        if val is not None:
            metadata[field] = val
    metadata["source"] = payload.get("source_domain") or "dressifye_saas"
